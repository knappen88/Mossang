...
namespace Building.UI
{
    ...
    public class BuildingMenuUI : MonoBehaviour
    {
        ...
        private int playerLevel = 1; // TODO: Получать из системы уровней
        ...
        private void CreateCategoryButtons()
        {
            ...
            foreach (BuildingCategory category in System.Enum.GetValues(typeof(BuildingCategory)))
            {
                ...
                BuildingCategory cat = category; // Захват переменной для замыкания
        ...
        {
            // Очищаем контейнер зданий
            foreach (Transform child in buildingsContainer)
            {
                Destroy(child.gameObject);
            }

            // Создаем слоты для зданий в категории
        ...
        private void CreateBuildingSlot(BuildingData building)
        {
            ...
            if (slot == null)
            {
                // Если нет компонента, настраиваем вручную
        ...
        private void ShowBuildingInfo(BuildingData building)
        {
            ...
            // Очищаем старые требования
            foreach (Transform child in requirementsContainer)
            {
                Destroy(child.gameObject);
            }

            // Показываем требования к ресурсам
        ...
        private void OnSelectBuilding()
        {
            ...
            // Передаем выбранное здание в систему строительства
